...
function validCookie(cookie, id, secret) {
  ...
  const c = cookie
    .split(";")
    .find(x => x.trim().startsWith("PARC_AUTH="));
...
function machinePage(id, machine) {
  const documents = machine.documents
    .map(
      ([file, label]) =>
        `<li>
          <a href="/document/${id}/${encodeURIComponent(file)}">
            ${label}
          </a>
        </li>`
    )
...
${
  documents
    ? `<ul>${documents}</ul>`
    : `<p>Aucun document n'est encore disponible pour ce matériel.</p>`
}
