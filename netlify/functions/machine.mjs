...
function docs(m,items){
  const by={}; for(const t of expectedTypes(m)) by[t]=[]; for(const x of items){if(!by[x.type])by[x.type]=[]; by[x.type].push(x)}
  const types=[...expectedTypes(m),...Object.keys(by).filter(t=>!expectedTypes(m).includes(t)&&TYPES[t])];
  let s='<h2>Documents</h2>';
  for(const t of types){const meta=TYPES[t], list=by[t]||[];s+=`<section><h3>${meta.label}</h3>`;if(!list.length)s+='<p class="muted">Document non chargé.</p>';for(const x of list){const a=alertState(x.expiry,t);s+=`<div class="doc"><a href="/document/${m.id}/${x.key.startsWith(m.id+'/') && x.key.split('/').length===2 ? 'legacy' : t}/${encodeURIComponent(x.key.split('/').pop())}">${esc(x.label||x.key)}</a>${x.expiry?` — <span class="${a.state}">${a.text} (${new Date(x.expiry+'T00:00:00').toLocaleDateString('fr-FR')})</span>`:(['carte','barreRouge','divers','doc','devis'].includes(t)?'':' — <span class="muted">Date non renseignée</span>')}</div>`}s+='</section>'}
