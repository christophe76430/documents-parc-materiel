...
async function employeeHome(drivers){
  ...
  const cards=enabled.map((d,i)=>{
    ...
    return `<a class="employee-card" href="/chauffeurs?id=${encodeURIComponent(d.id)}"><span class="employee-avatar"><img src="${src}" alt="Photo de ${esc(d.name)}" onerror="this.style.display='none';this.parentElement.classList.add('avatar-fallback');this.parentElement.textContent='${esc((d.name||'?').trim().charAt(0).toUpperCase())}'"></span><span class="employee-card-main"><strong>${esc(d.name)}</strong><small>Accéder à mon espace</small></span><span class="employee-arrow">→</span></a>`;
  }).join('');
  const rows=[];
