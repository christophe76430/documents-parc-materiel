...
function page(docs,drivers,msg='',statuses={},extDates={},archives=[],alerts={}){
    ...
    return `<div class="machine-status-row ${enabled?'':'is-off'}">
      <div class="machine-status-switch-wrap">
        <form method="post" action="/.netlify/functions/admin" class="machine-status-form">
          <input type="hidden" name="action" value="toggle-machine"><input type="hidden" name="id" value="${esc(id)}"><input type="hidden" name="enabled" value="${enabled?'0':'1'}">
          <button type="submit" class="machine-switch ${enabled?'is-on':'is-off'}" aria-label="${enabled?'Désactiver':'Activer'} ${esc(code)}"><span class="machine-switch-track"><span class="machine-switch-thumb"></span></span><span class="machine-switch-label">${enabled?'ON':'OFF'}</span></button>
        </form>
      </div>
  ...
  const driverHtml=drivers.length?drivers.map(d=>`<div class="driver-admin-card"><div class="driver-admin-head"><strong>👷 ${esc(d.name)}</strong><span class="muted">Code personnel enregistré</span></div><form method="post" action="/.netlify/functions/admin" enctype="multipart/form-data"><input type="hidden" name="action" value="upload-driver"><input type="hidden" name="driver" value="${esc(d.id)}"><div class="driver-upload-grid"><div><label>Rubrique</label><select name="cat">${Object.entries(DRIVER_CATEGORIES).map(([k,v])=>`<option value="${k}">${v.label}</option>`).join('')}</select></div><div><label>Fichier</label><input type="file" name="file" accept=".pdf,.jpg,.jpeg,.png" required></div></div><button>Charger le document chauffeur</button></form></div>`).join(''):'<p class="muted">Aucun chauffeur enregistré.</p>';

  const docsHtml=docs.map(d=>`<div class="doc-card searchable" data-search="${esc((MACHINES[d.id].name+' '+d.label).toLowerCase())}"><span><b>${esc(MACHINES[d.id].name)}</b><br><span class="muted">${esc(TYPES[d.type]?.label||d.type)}</span><br>${esc(d.label)}</span><span class="doc-actions"><form method="post" action="/.netlify/functions/admin" enctype="multipart/form-data" class="replace-form"><input type="hidden" name="action" value="replace-doc"><input type="hidden" name="key" value="${esc(d.key)}"><input type="hidden" name="id" value="${esc(d.id)}"><input type="hidden" name="type" value="${esc(d.type)}"><input type="file" name="file" accept=".pdf,.jpg,.jpeg,.png" required hidden onchange="this.form.submit()"><button type="button" class="replace" onclick="this.previousElementSibling.click()">Remplacer</button></form><form method="post" action="/.netlify/functions/admin" onsubmit="return confirm('Supprimer définitivement ce document ?')"><input type="hidden" name="action" value="delete-doc"><input type="hidden" name="key" value="${esc(d.key)}"><button class="danger">Supprimer</button></form></span></div>`).join('')||'<p>Aucun document.</p>';

  const archiveHtml=archives.length?archives.map(a=>`<div class="doc-card archive-card"><span><b>${esc(MACHINES[a.id]?.name||a.id)}</b><br><span class="muted">${esc(TYPES[a.type]?.label||a.type)}</span><br>${esc(a.label)}${a.archivedAt?`<br><small class="muted">Archivé le ${esc(new Date(a.archivedAt).toLocaleString('fr-FR'))}</small>`:''}</span><span class="doc-actions"><a class="replace" href="/.netlify/functions/archive?key=${encodeURIComponent(a.key)}" target="_blank" rel="noopener">👁️ Consulter</a></span></div>`).join(''):'<p class="muted">Aucun document archivé.</p>';

  return shell(`<style>
  .admin-page{max-width:1280px;margin:auto}.admin-top{display:flex;justify-content:space-between;align-items:center;gap:16px;flex-wrap:wrap}.admin-nav{display:flex;gap:8px;flex-wrap:wrap;margin:14px 0}.admin-nav a{display:inline-block;padding:9px 13px;border-radius:9px;background:#edf4fa;color:#173b61;text-decoration:none;font-weight:600}.admin-nav a:hover{background:#dceaf5}.admin-section{margin:14px 0;border:1px solid #d8e4ee;border-radius:14px;background:#fff;overflow:hidden}.admin-section>summary{cursor:pointer;list-style:none;padding:16px 18px;font-size:20px;font-weight:700;color:#173b61;background:#f7fafc}.admin-section>summary::-webkit-details-marker{display:none}.admin-section>summary:before{content:'＋';display:inline-block;width:26px;color:#1976d2}.admin-section[open]>summary:before{content:'−'}.admin-section-content{padding:16px 18px}.quick-actions{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.quick-action{padding:14px;border-radius:10px;background:#f4f8fb;text-decoration:none;color:#173b61;font-weight:700}.quick-action small{display:block;margin-top:4px;font-weight:400;color:#526b80}.driver-admin-card{border:1px solid #dbe6ef;border-radius:10px;padding:14px;margin:10px 0}.driver-admin-head{display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;margin-bottom:10px}.driver-upload-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.admin-search{margin-bottom:12px}.archive-count{font-weight:700;color:#1976d2}@media(max-width:800px){.quick-actions{grid-template-columns:1fr 1fr}.driver-upload-grid{grid-template-columns:1fr}}@media(max-width:520px){.quick-actions{grid-template-columns:1fr}}
  </style><main class="admin-page">
  <div class="box"><div class="admin-top"><div><p><a href="/">← Accueil</a></p><h1>🔐 Administration THN</h1><p class="muted">Gestion du parc, documents, chauffeurs et archives.</p></div><a class="replace" href="/export">📊 Export Excel</a></div>${msg?`<p class="ok">${esc(msg)}</p>`:''}</div>
  ...
  <nav class="admin-nav" aria-label="Accès rapide"><a href="#parc">⚙️ Parc</a><a href="#documents">📄 Documents</a><a href="#chauffeurs">👷 Chauffeurs</a><a href="#archives">🗄️ Archives</a></nav>

  <details class="admin-section" id="parc" open><summary>⚙️ État du parc</summary><div class="admin-section-content"><p class="muted">Mets un matériel sur <strong>OFF</strong> lorsqu'il est au garage ou inutilisé. Ses échéances disparaissent de l'accueil et des listes d'échéances.</p>${fleetHtml}</div></details>

  <details class="admin-section" id="documents" open><summary>📄 Documents</summary><div class="admin-section-content">
    <div class="quick-actions"><a class="quick-action" href="#import-document">📄 Ajouter un document<small>Importer un fichier pour un engin</small></a><a class="quick-action" href="#import-parc">📦 Importer tout le parc<small>Importer le dossier PARCMAT</small></a><a class="quick-action" href="#liste-documents">🔎 Rechercher un document<small>Remplacer ou supprimer</small></a><a class="quick-action" href="#archives">🗄️ Voir les archives<small>${archives.length} fichier(s) archivé(s)</small></a></div>
    <div class="admin-section" id="import-document"><div class="admin-section-content"><h3>📄 Ajouter un document</h3><form method="post" action="/.netlify/functions/admin" enctype="multipart/form-data"><label>Matériel</label><select name="id">${Object.values(MACHINES).map(m=>`<option value="${m.id}">${esc(m.name)}</option>`).join('')}</select><label>Type</label><select name="type" id="doc-type">${Object.entries(TYPES).map(([k,v])=>`<option value="${k}">${v.label}</option>`).join('')}</select><label id="expiry-label">Date d'expiration</label><input id="expiry" type="date" name="expiry"><label>Fichier</label><input type="file" name="file" accept=".pdf,.jpg,.jpeg,.png" required><button>Charger le document</button></form></div></div>
    <div class="admin-section" id="import-parc"><div class="admin-section-content"><h3>📦 Importer tout le parc</h3><p class="muted">Sélectionne le dossier <strong>PARCMAT</strong>. Les sous-dossiers sont reconnus automatiquement.</p><label>Mot de passe administrateur</label><input id="bulk-password" type="password"><label>Dossier PARCMAT</label><input id="bulk-files" type="file" webkitdirectory directory multiple accept=".pdf,.jpg,.jpeg,.png"><button type="button" id="bulk-start">Importer tout le parc</button><div id="bulk-status" class="muted"></div><pre id="bulk-log"></pre></div></div>
    <div class="admin-section" id="liste-documents"><div class="admin-section-content"><h3>🔎 Documents du parc</h3><input class="admin-search" id="doc-search" type="search" placeholder="Rechercher un matériel ou document...">${docsHtml}</div></div>
  </div></details>

  <details class="admin-section" id="chauffeurs"><summary>👷 Chauffeurs</summary><div class="admin-section-content"><p class="muted">Gérer les accès personnels et charger leurs documents.</p><h3>Ajouter un chauffeur</h3><form method="post" action="/.netlify/functions/admin"><label>Nom du chauffeur</label><input name="name" required><label>Code personnel</label><input name="code" required><button>Ajouter le chauffeur</button></form><h3>Chauffeurs enregistrés</h3>${driverHtml}</div></details>

  <details class="admin-section" id="archives"><summary>🗄️ Archives <span class="archive-count">${archives.length}</span></summary><div class="admin-section-content"><p class="muted">Les anciens fichiers sont conservés automatiquement lors d’un remplacement.</p>${archiveHtml}</div></details>

  <script>const t=document.getElementById('doc-type'),e=document.getElementById('expiry'),no=${JSON.stringify([...NO_EXPIRY])};function sync(){if(!t||!e)return;e.style.display=no.includes(t.value)?'none':''}t?.addEventListener('change',sync);sync();const q=document.getElementById('doc-search');q?.addEventListener('input',()=>document.querySelectorAll('.searchable').forEach(x=>x.style.display=!q.value||x.dataset.search.includes(q.value.toLowerCase())?'':'none'));
  document.querySelectorAll('.machine-status-form').forEach(form=>form.addEventListener('submit',async ev=>{ev.preventDefault();const btn=form.querySelector('.machine-switch');if(!btn||btn.disabled)return;const row=form.closest('.machine-status-row');const state=row?.querySelector('.machine-status-state');const label=btn.querySelector('.machine-switch-label');const hidden=form.querySelector('input[name=enabled]');btn.disabled=true;try{const r=await fetch('/api/machine-status',{method:'POST',body:new FormData(form),credentials:'same-origin',headers:{'Accept':'application/json'}});const d=await r.json();if(!r.ok||!d.ok)throw new Error(d?.message||'Erreur');const on=!!d.enabled;row?.classList.toggle('is-off',!on);btn.classList.toggle('is-on',on);btn.classList.toggle('is-off',!on);btn.setAttribute('aria-label',(on?'Désactiver':'Activer')+' '+(form.querySelector('input[name=id]')?.value||''));if(label)label.textContent=on?'ON':'OFF';if(hidden)hidden.value=on?'0':'1';if(state)state.textContent=on?'ON — échéances prises en compte':'OFF — échéances masquées';showStatus(d.message);}catch(err){showStatus(err.message||'Erreur lors de la modification.',true);}finally{btn.disabled=false;}}));function showStatus(message,bad=false){let box=document.getElementById('fleet-status-message');if(!box){box=document.createElement('div');box.id='fleet-status-message';box.style.cssText='margin:10px 0;padding:10px 12px;border-radius:8px;background:#eef6ff;color:#123;';document.querySelector('.admin-section-content')?.prepend(box)}box.textContent=message;box.style.background=bad?'#fff0f0':'#eef6ff';setTimeout(()=>{if(box)box.textContent='';},2500);}</script><script src="/bulk-import.js"></script>
  </main>`);
}
