...
function page(docs,drivers,msg='',statuses={},extDates={},archives=[],alerts={}){
  ...
  <div class="box"><h2>👷 Gestion des chauffeurs / salariés</h2>${drivers.map(d=>`<div class="driver-admin-row"><div><p><b>${esc(d.name)}</b> — code personnel enregistré</p><form method="post" action="/.netlify/functions/admin" class="medical-date-form"><input type="hidden" name="action" value="set-medical-visit"><input type="hidden" name="driver" value="${esc(d.id)}"><label>Prochain RDV de visite médicale</label><input type="date" name="medicalVisitDate" value="${esc(d.medicalVisitDate||'')}"><button type="submit">Enregistrer</button></form></div></div>`).join('')||'<p class="muted">Aucun salarié.</p>'}<hr><form method="post" action="/.netlify/functions/admin"><input type="hidden" name="action" value="add-driver"><label>Nom du salarié</label><input name="name" required><label>Code personnel</label><input name="code" required><button>Ajouter le salarié</button></form></div>${drivers.map(d=>`<div class="box"><h3>📁 Documents — ${esc(d.name)}</h3><form method="post" action="/.netlify/functions/admin" enctype="multipart/form-data"><input type="hidden" name="action" value="upload-driver"><input type="hidden" name="driver" value="${esc(d.id)}"><label>Rubrique</label><select name="cat">${Object.entries(DRIVER_CATEGORIES).map(([k,v])=>`<option value="${k}">${v.label}</option>`).join('')}</select><label>Fichier</label><input type="file" name="file" accept=".pdf,.jpg,.jpeg,.png" required><button>Charger</button></form></div>`).join('')}
