...
export default async req=>{
  try{
    ...
    if(action==='upload-driver'){
      ...
      await driverStore().set(`docs/${did}/${cat}/${file.name.replace(/[\\/]/g,'_')}`,await file.arrayBuffer(),{metadata:{label:DRIVER_CATEGORIES[cat].label,contentType:file.type,uploadedAt:new Date().toISOString(),...(expiry?{expiry}: {})}});
