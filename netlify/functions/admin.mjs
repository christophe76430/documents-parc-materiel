...
export default async req=>{
  try{
    ...
    if(action==='upload-driver'){
      const did=String(fd.get('driver')||''),cat=String(fd.get('cat')||'divers'),file=fd.get('file'),expiry=String(fd.get('driverExpiry')||'').trim();
      if(!did||!DRIVER_CATEGORIES[cat]||!(file instanceof File))return html(shell('<p class="bad">Données invalides.</p>'),400);
      await driverStore().set(`docs/${did}/${cat}/${file.name.replace(/[\\/]/g,'_')}`,await file.arrayBuffer(),{metadata:{label:DRIVER_CATEGORIES[cat].label,contentType:file.type,expiry:/^\d{4}-\d{2}-\d{2}$/.test(expiry)?expiry:'',uploadedAt:new Date().toISOString()}});
