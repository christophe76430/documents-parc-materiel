...
export default async req=>{
  try{
    ...
    if(action==='change-driver-code'){
      const driverId=String(fd.get('driver')||'');
      const code=String(fd.get('code')||'').trim();
      if(!driverId||!code)return html(shell('<p class="bad">Chauffeur et nouveau code obligatoires.</p>'),400);
      const key=`driver/${driverId}.json`;
      const d=await driverStore().get(key,{type:'json'}).catch(()=>null);
      if(!d)return html(shell('<p class="bad">Chauffeur introuvable.</p>'),404);
      d.codeHash=hashSecret(code);
      delete d.code;
      await driverStore().set(key,JSON.stringify(d),{metadata:{type:'driver'}});
      const data=await loadAdminData();
      return html(page(data.docs,data.drivers,`Code secret de ${d.name} modifié.`,data.statuses,data.extDates,data.archives,data.alerts));
    }
